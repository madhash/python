...
############################################################################################################
def sub(l):
    x=len(l);s=[];i=0;t=['-','*'];tar=l
    for i in range(x):
        
        if(i!=0 and i!=x-1 and l[i] in t and (l[i+1]not in t) and (l[i-1]not in t)):
...
def mul(l):
    ...
    for i in range(x):
        print i
        if(i!=0 and i!=x-1 and l[i]=='*' and (l[i+1] not in t) and (l[i-1] not in t)):
            print l[i+1]
            l[i]=l[i+1]*l[i-1]
            l[i-1]=0;l[i+1]=0
            print l
...
class TwentyFortyEight:
    ...
    def new_tile(self):
        ...
        s=['-','*','*']
        l=[r.choice(l),r.choice(l),r.choice(s)]
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                if(self.n[i][j]!=0):
                    sum+=1
        if(sum==self.grid_height*self.grid_width):
            return self.n
...
def game_start():
    ...
    while(1):
        if(z==0):
            print  "One second please ! press i for instructions or key in 'e' to exit the game if you feel borred "
            print "UP:1    DOWN:2    LEFT:3    RIGHT:4"
            q=raw_input("Set your target master ----->")
